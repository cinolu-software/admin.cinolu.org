...
import { FilterOpportunityTagsDto } from '../dto/tags/filter-tags.dto';
import { buildQueryParams } from '@shared/helpers';
import { ITag } from '@shared/models';
import { ToastrService } from '@shared/services/toast/toastr.service';
import { OpportunityTagDto } from '../dto/tags/opportunity-tag.dto';

interface IOpportunityTagsStore {
  isLoading: boolean;
  tags: [ITag[], number];
  allTags: ITag[];
  lastQuery: FilterOpportunityTagsDto | null;
}

export const OpportunityTagsStore = signalStore(
  withState<IOpportunityTagsStore>({ isLoading: false, allTags: [], tags: [[], 0], lastQuery: null }),
  withProps(() => ({
  ...
  withMethods(({ _http, _toast, ...store }) => ({
    loadUnpaginated: rxMethod<void>(
      pipe(
        ...
        exhaustMap(() => {
          return _http.get<{ data: ITag[] }>('opportunity-tags').pipe(
            map(({ data }) => {
    ...
    loadFiltered: rxMethod<FilterOpportunityTagsDto>(
      pipe(
        tap(() => patchState(store, { isLoading: true })),
        switchMap((queryParams) => {
          patchState(store, { lastQuery: queryParams });
          const params = buildQueryParams(queryParams);
          return _http.get<{ data: [ITag[], number] }>('opportunity-tags/filtered', { params }).pipe(
            map(({ data }) => {
    ...
    create: rxMethod<{ payload: OpportunityTagDto; onSuccess: () => void }>(
      pipe(
        tap(() => patchState(store, { isLoading: true })),
        switchMap(({ payload, onSuccess }) =>
          _http.post<{ data: ITag }>('opportunity-tags', payload).pipe(
            map(({ data }) => {
              const [tags, count] = store.tags();
              patchState(store, { tags: [[data, ...tags], count + 1] });
              _toast.showSuccess('Tag ajouté avec succès');
    ...
    update: rxMethod<{ id: string; payload: { name: string }; onSuccess: () => void }>(
      pipe(
        ...
        switchMap(({ id, payload, onSuccess }) =>
          _http.patch<{ data: ITag }>(`opportunity-tags/${id}`, payload).pipe(
            map(({ data }) => {
              const [tags, count] = store.tags();
              const updated = tags.map((t) => (t.id === data.id ? data : t));
              patchState(store, { tags: [updated, count] });
              _toast.showSuccess('Tag mis à jour');
              patchState(store, { isLoading: false });
              onSuccess();
            }),
            catchError(() => {
    ...
    delete: rxMethod<{ id: string }>(
      pipe(
        tap(() => patchState(store, { isLoading: true })),
        switchMap(({ id }) =>
          _http.delete<void>(`opportunity-tags/${id}`).pipe(
            map(() => {
              const [tags, count] = store.tags();
              const filtered = tags.filter((tag) => tag.id !== id);
              patchState(store, { tags: [filtered, count - 1] });
              patchState(store, { isLoading: false });
              _toast.showSuccess('Tag supprimé avec succès');
            }),
            catchError(() => {
              patchState(store, { isLoading: false });
